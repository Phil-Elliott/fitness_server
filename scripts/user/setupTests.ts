...
async function setupUserTableForTestDatabase() {
  try {
    ...
    await db.execute(
      sql`INSERT INTO users (id, email, display_name) VALUES ('user_13543', 'bobsmith@gmail.com', 'Bob Smith');`
    );
    await db.execute(
      sql`INSERT INTO users (id, email, display_name) VALUES ('user_1257753', 'elizebethOwens@aol.com', 'Elizebeth Owens');`
    );
  } catch (error) {
